...
const REPORT_THEMES = {
  ...
  monthly: {
    ...
    quotes: [
      ...
      '"Each month is a mini-chapter in your story."',
  ...
  transit: {
    ...
    steps: [
      ...
      { icon: '✦', label: 'Writing your transit story', sub: 'Connecting this week\'s sky to your chart...' },
...
const REPORTS = [
  ...
  { icon: '☽', bg: ['#1A0A3A', '#3A1A28'], accent: '#A080E0', name: 'Lunar Guide', desc: 'Moon rituals aligned with your natal chart', type: 'lunar', tier: 'pro' },
  ...
  { icon: '♃', bg: ['#0A1A2A', '#3A1A28'], accent: '#4ECDC4', name: 'Year-Ahead Forecast', desc: `Month-by-month roadmap for ${CURRENT_YEAR}`, type: 'yearly', tier: 'pro' },
  ...
  { icon: '☉', bg: ['#3A1A28', '#2A1A6E'], accent: '#C8A84B', name: `Solar Return ${CURRENT_YEAR}`, desc: 'Your complete year ahead from birthday to birthday', type: 'solar_return', tier: 'pro' },
];
...
.cover-foot-byline {
  font-size: 7px; color: rgba(196,154,42,0.45);
  letter-spacing: 1.2px; margin-top: 4px; font-style: italic;
  font-family: Helvetica, Arial, sans-serif;
}
...
<div class="cover">
  ...
  <div class="cover-foot">
    ...
    <div class="cover-foot-byline">Synthesized from established Western astrology traditions</div>
...
<div class="closing">
  ...
  <div class="closing-brand-section">
    ...
    <div class="closing-disclaimer">
      Calculated using astronomy-engine + IAU ephemeris.<br>
...
export default function ReportsScreen() {
  ...
  const getReportDescription = (type) => {
    const defaults = {
      ...
      monthly: 'This month\'s forecast in detail',
  ...
  const handleReport = async (r) => {
    ...
    capture(EVENTS.REPORT_VIEWED, { report_type: r.type, source: 'reports_grid' });
    try {
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 112 }}>
        <View style={styles.top}>
          ...
          <Text style={[styles.sub, { color: colors.textSecondary }]}>
            {sunSign
              ...
              : 'Premium chart intelligence, written by AI using your exact birth chart.'}
          </Text>
        </View>

        {/* Featured — Monthly Forecast (auto-updates each month) */}
        <TouchableOpacity style={[styles.featuredWrap, { shadowColor: isDark ? 'transparent' : '#000' }]} activeOpacity={0.85} onPress={handleMonthlyReport}>
          ...
          <View style={[styles.featuredBody, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.featuredDesc, { color: colors.textSecondary }]}>
              {sunSign
                ...
                : `${MONTH_ZODIAC_ENERGY[CURRENT_MONTH]} Your personalized week-by-week guide.`}
            </Text>
        ...
        <View style={styles.grid}>
          {REPORTS.map((r, i) => (
            <TouchableOpacity key={i} style={[styles.rtile, { backgroundColor: colors.card, borderColor: colors.border, shadowColor: isDark ? 'transparent' : '#000' }]} activeOpacity={0.8} onPress={() => handleReport(r)}>
              <LinearGradient colors={r.bg} style={styles.rtileColor}>
                <Text style={{ fontSize: 32, color: r.accent }}>{r.icon}</Text>
                {!isReportAccessible(r.type, isPro) && (
      ...
      <Modal visible={reportModal} animationType="slide" presentationStyle="pageSheet">
        <View style={[styles.modal, { backgroundColor: colors.modalBg }]}>
          ...
          {reportLoading ? (
            <View style={[styles.loadingWrap, { padding: 24 }]}>
              {/* Skeleton paragraphs while the report generates — replaces the
                  spinner with shape-correct placeholders so layout doesn't shift
                  when content lands. */}
              <Skeleton width="50%" height={11} radius={4} style={{ marginBottom: 18, alignSelf: 'flex-start' }} />
              <Skeleton width="90%" height={26} radius={6} style={{ marginBottom: 22 }} />
              <Skeleton width="100%" height={14} radius={4} style={{ marginBottom: 8 }} />
              <Skeleton width="96%" height={14} radius={4} style={{ marginBottom: 8 }} />
              <Skeleton width="88%" height={14} radius={4} style={{ marginBottom: 22 }} />
              <Skeleton width="35%" height={11} radius={4} style={{ marginBottom: 14, alignSelf: 'flex-start' }} />
              <Skeleton width="100%" height={14} radius={4} style={{ marginBottom: 8 }} />
              <Skeleton width="92%" height={14} radius={4} style={{ marginBottom: 8 }} />
              <Skeleton width="78%" height={14} radius={4} style={{ marginBottom: 22 }} />
              <Text style={[styles.loadingText, { color: colors.heading, fontSize: 14, marginTop: 8 }]}>Reading your transits…</Text>
              <Text style={[styles.loadingSubtext, { color: colors.textSecondary }]}>Drafting a read written for your chart</Text>
            </View>
          ) : reportData ? (
            <>
              <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
                {/* Summary */}
                <LinearGradient colors={['#3A1A28', '#1A1060']} style={styles.summaryCard}>
                ...
                {reportData.keyInsight && (
                  <LinearGradient colors={['#2A1A6E', '#3A1A28']} style={styles.insightCard}>
