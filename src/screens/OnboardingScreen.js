...
export default function OnboardingScreen({ navigation }) {
  ...
  const handleContinue = async () => {
    if (step === 1) {
      ...
      // Final step: save profile & calculate chart
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        {step === 1 && (
          <>
            <Text style={styles.h1}>Tell us when you{'\n'}<Text style={styles.h1em}>entered the world</Text></Text>
            <Text style={styles.body}>Your exact birth details allow us to cast a precise natal chart — the celestial blueprint of your entire life.</Text>

            <View style={styles.fgroup}>
              <Text style={styles.flabel}>YOUR NAME</Text>
              <View style={[styles.finput, name.length > 0 && styles.finputFilled]}>
                <TextInput
                  style={styles.finputText}
                  value={name}
            ...
            <View style={styles.frow}>
              <View style={[styles.fgroup, { flex: 1 }]}>
                ...
                <TouchableOpacity style={[styles.finput, birthDate && styles.finputFilled]} onPress={() => setShowDatePicker(true)}>
                  <Text style={birthDate ? styles.finputVal : styles.finputPh}>
              ...
              <View style={{ width: 11 }} />
              <View style={[styles.fgroup, { flex: 1 }]}>
                <Text style={styles.flabel}>BIRTH TIME</Text>
                <TouchableOpacity
                  style={[styles.finput, (!isTimeUnknown && birthTime) && styles.finputFilled]}
                  onPress={() => !isTimeUnknown && setShowTimePicker(true)}
                >
                  <Text style={(isTimeUnknown || birthTime) ? styles.finputVal : styles.finputPh}>
            ...
            <TouchableOpacity style={styles.unknownRow} onPress={() => setIsTimeUnknown(!isTimeUnknown)}>
              <View style={[styles.checkbox, isTimeUnknown && styles.checkboxOn]}>
                {isTimeUnknown && <Text style={styles.checkmark}>✓</Text>}
              </View>
              <Text style={styles.unknownText}>I don't know my exact birth time</Text>
            </TouchableOpacity>

            <View style={styles.fgroup}>
              <Text style={styles.flabel}>CITY OF BIRTH</Text>
              <View style={[styles.finput, selectedLocation && styles.finputFilled]}>
                <TextInput
                  style={styles.finputText}
                  value={locationQuery}
              ...
              {!locationSearching && citySuggestions.length > 0 && (
                <View style={styles.suggestions}>
                  {citySuggestions.map((city, i) => (
                    <TouchableOpacity key={i} style={styles.suggestion} onPress={() => selectCity(city)}>
                      <Text style={styles.suggestionText} numberOfLines={2}>{city.name}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              )}
            ...
            <View style={styles.hint}>
              ...
              <Text style={styles.hintTxt}>Birth time determines your Rising sign and house placements. If unknown, we'll calculate a noon chart and note reduced accuracy.</Text>
            </View>
        ...
        {step === 2 && (
          <>
            <Text style={styles.h1}>How should your{'\n'}<Text style={styles.h1em}>stars speak?</Text></Text>
            <Text style={styles.body}>Choose the voice that resonates with you. You can always change this later in settings.</Text>

            <View style={styles.pgrid}>
              {personas.map((p, i) => (
                <TouchableOpacity key={i} style={[styles.pcard, persona === i && styles.pcardOn]} onPress={() => setPersona(i)} activeOpacity={0.8}>
                  ...
                  <Text style={styles.pcardName}>{p.name}</Text>
                  <Text style={styles.pcardDesc}>{p.desc}</Text>
                </TouchableOpacity>
              ))}
            </View>

            <View style={styles.dtoggle}>
              {['Beginner', 'Intermediate', 'Advanced'].map((d, i) => (
                <TouchableOpacity key={i} style={[styles.dbtn, depth === i && styles.dbtnOn]} onPress={() => setDepth(i)}>
                  <Text style={[styles.dbtnText, depth === i && styles.dbtnTextOn]}>{d}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </>
        ...
        {step === 3 && (
          <>
            <Text style={styles.h1}>Stay aligned with{'\n'}<Text style={styles.h1em}>the cosmos</Text></Text>
            <Text style={styles.body}>We'll send you gentle cosmic nudges — daily horoscopes, transit alerts, and lunar phase updates. Never spammy, always meaningful.</Text>

            {[
              { icon: '☀️', title: 'Daily Horoscope', desc: 'Your personalized morning reading', on: true },
              { icon: '🌙', title: 'Moon Phases', desc: 'New & full moon rituals', on: true },
              { icon: '⚡', title: 'Transit Alerts', desc: 'When planets hit your chart', on: false },
              { icon: '💫', title: 'Weekly Forecast', desc: 'Sunday evening preview', on: true },
            ].map((item, i) => (
              <View key={i} style={styles.notifRow}>
                <View style={[styles.notifIcon, { backgroundColor: i % 2 === 0 ? '#F2ECF8' : '#FFF2E4' }]}>
                ...
                <View style={[styles.toggleTrack, item.on && styles.toggleOn]}>
