...
export const UserProfileProvider = ({ children }) => {
    ...
    // V1.2 — Re-readable load function. Originally this was only an inline IIFE
    // that ran once on mount; extracting it lets demoData (or any future hot-
    // reload path) force a fresh re-read from SQLite without unmounting the
    // provider. The legacy AsyncStorage migration only runs when SQLite is
    // empty, so re-running this function on a populated DB just re-syncs
    // React state to SQLite (which is what we want after a wipe + reseed).
        ...
        try {
            ...
            if (allProfiles.length > 0) {
                ...
                if (legacyProfile) {
                    ...
                    // SQLite empty AND no legacy — explicitly clear React state
                    // so a previous session's stale profile doesn't persist
                    // after a wipe.
                    setUserProfileState(null);
                    setPartnerProfilesState([]);
                }
            }
        } catch (e) {
    ...
    // Public method: callers (e.g. demoData via Profile screen) invoke this
    // after they've directly mutated SQLite, so React state catches up.
