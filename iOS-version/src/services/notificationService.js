...
// Channel IDs kept stable (storage compat). Display names softened for v1
// to avoid 4.3(b) trigger words in the iOS Settings → Notifications shelf.
const CHANNELS = [
  { id: 'cosmic_morning', name: 'Morning Briefing', description: 'Your daily relational briefing', importance: Notifications.AndroidImportance?.HIGH },
  { id: 'evening_reflection', name: 'Evening Reflection', description: 'Nightly journal prompts and weekly recaps', importance: Notifications.AndroidImportance?.DEFAULT },
  { id: 'transit_alerts', name: 'Pattern Alerts', description: 'Significant pattern shifts in the week ahead', importance: Notifications.AndroidImportance?.HIGH },
  { id: 'streak_guardian', name: 'Streak Reminders', description: 'Protect your daily streak', importance: Notifications.AndroidImportance?.DEFAULT },
  { id: 'cosmic_milestones', name: 'Milestones', description: 'Badge unlocks and level-ups', importance: Notifications.AndroidImportance?.DEFAULT },
  { id: 'weekly_digest', name: 'Weekly Digest', description: 'Your week ahead', importance: Notifications.AndroidImportance?.DEFAULT },
];
  ...
  for (const ch of CHANNELS) {
    try {
      await Notifications.setNotificationChannelAsync(ch.id, {
        ...
        // expo-notifications SDK 54+: sound must be `false` (silent) or a string;
        // `null` throws "Cannot cast 'nil' for field 'sound'".
        sound: false,
    ...
    for (const item of filtered) {
      try {
        const notifContent = {
          ...
          // expo-notifications SDK 54+: must be `false` (silent) or a sound string.
          sound: false,
