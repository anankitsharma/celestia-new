...
// Wipes AsyncStorage + drops/recreates SQLite tables, returning the device
// to a fresh-install state. Required by 5.1.1(v) for non-authed apps.
export const resetAllAppData = async () => {
    // Clear AsyncStorage
    try { await AsyncStorage.clear(); }
    catch (e) { console.warn('[reset] AsyncStorage clear failed:', e); }

    // Drop SQLite tables and recreate via initSchema
    try {
        ...
        const tables = [
            ...
            'forecasts', 'reports', 'charts', 'profiles',
            'journal_entries', 'achievements', 'user_xp', 'user_streaks',
        ];
        for (const t of tables) {
            try { await db.execAsync(`DROP TABLE IF EXISTS ${t};`); }
            catch (e) { console.warn(`[reset] drop ${t} failed:`, e); }
        }
        await initSchema();
    } catch (e) {
        console.warn('[reset] SQLite reset failed:', e);
    }
};
