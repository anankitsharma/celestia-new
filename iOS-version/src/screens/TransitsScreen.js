  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <LinearGradient colors={['#EEF1F2', '#E8ECEE', '#E2E7EA']} locations={[0, 0.5, 1]} style={styles.hero}>
          <View style={styles.heroGlow} />
          {navigation.canGoBack() && (
            <TouchableOpacity onPress={() => navigation.goBack()} style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 10 }}>
              <Text style={{ fontSize: 16, color: T.inkDim }}>‹</Text>
              <Text style={{ fontSize: 13, color: T.inkDim, fontFamily: FONTS.sans }}>Back</Text>
            </TouchableOpacity>
          )}
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <Text style={styles.title}>Today's Outlook</Text>
            <CosmicTooltip id="transit" size={16} />
          </View>
          <Text style={styles.sub}>
            {loading
              ? 'Reading the sky…'
              : `${transits.length} pattern${transits.length !== 1 ? 's' : ''} active right now`
            }
          </Text>
          ...
          {cosmicSeason && (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 10, backgroundColor: 'rgba(184,153,104,0.14)', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 }}>
              <Text style={{ fontSize: 10, color: T.inkDim, fontFamily: FONTS.sansSemiBold, letterSpacing: 1 }}>YOUR SEASON</Text>
              <Text style={{ fontSize: 11, color: T.ink, fontFamily: FONTS.sans }}>{cosmicSeason.planet} in {cosmicSeason.natalTarget} · {cosmicSeason.progress}%</Text>
            </View>
          )}
        </LinearGradient>
        ...
        {mercuryRxData && (
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            onPress={() => navigation.navigate('Main', { screen: 'Ask', params: { initialMessage: `Why does communication feel especially off right now? What should I watch out for this week?` } })}>
            <View style={styles.rxCompactLeft}>
              <Text style={styles.rxCompactGlyph}>☿</Text>
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  <Text style={[styles.rxCompactTitle, { color: isDark ? '#E8A840' : '#8B6A28' }]}>Communication Friction</Text>
                  ...
                  <View style={[styles.tcardBody, { borderTopColor: colors.divider, backgroundColor: isDark ? colors.cardAlt : '#FDFAF6' }]}>
                    ...
                    {aiLoading[i] && (
                      <View style={styles.taiLoadingRow}>
                        ...
                        <Text style={[styles.taiLoadingText, { color: colors.textSecondary }]}>Reading your patterns...</Text>
                      </View>
                    )}
                    {/* V1: paywall lock removed — all users see AI insight when ready */}
                    {(
                      aiInsights[i] && (
                    ...
                    <View style={styles.tbodyMeta}>
                      ...
                      <View style={{ flexDirection: 'row', gap: 6 }}>
                        ...
                        <TouchableOpacity style={styles.tbodyAi}
                          onPress={() => navigation.navigate('Main', { screen: 'Ask', params: { initialMessage: `Tell me about the transit ${t.aspect} happening right now. What does it mean for me?` } })}>
        ...
        {upcomingEvents.length > 0 && (
          <View style={styles.timelineSection}>
            ...
            <Text style={[styles.timelineSub, { color: colors.textSecondary }]}>Notable patterns in the next 2 weeks</Text>
...
const styles = StyleSheet.create({
  ...
  heroGlow: { position: 'absolute', width: 260, height: 260, borderRadius: 130, backgroundColor: 'rgba(92,36,52,0.05)', left: -80, top: -40 },
  title: { fontFamily: FONTS.serif, fontSize: 30, color: T.ink, marginBottom: 5, letterSpacing: -0.4 },
  sub: { fontSize: 12.5, color: T.inkDim, marginBottom: 4, fontFamily: FONTS.sans },
  skyStrip: { marginTop: 14, backgroundColor: T.surface, borderWidth: 1, borderColor: T.hairline, borderRadius: 12, padding: 12, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  skyStripItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  skyStripGlyph: { fontSize: 16, color: T.clay },
  skyStripLabel: { fontSize: 11, color: T.ink, fontFamily: FONTS.sansMedium },
  skyStripDot: { width: 3, height: 3, borderRadius: 1.5, backgroundColor: 'rgba(184,153,104,0.6)' },
  skyStripAspect: { fontSize: 10, color: T.clay, fontFamily: FONTS.sansSemiBold, letterSpacing: 1, textTransform: 'uppercase' },
