  ...
  const renderEntry = () => {
    ...
    return (
      <View style={[s.entryCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        {/* Cosmic Snapshot — "The Sky When You Wrote This" — opt-in only */}
        {showAstrology && snapshot && snapshot.moon && (
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <View style={[s.header, { borderBottomColor: colors.border }]}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('Journal')} style={s.writeBtn}
          ...
          <Text style={s.writeText} accessibilityElementsHidden importantForAccessibility="no-hide-descendants">✍</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={s.monthNav}>
          <TouchableOpacity onPress={prevMonth} style={[s.navBtn, { backgroundColor: colors.card, borderColor: colors.border }]}
            ...
            <Text style={[s.navBtnText, { color: colors.heading }]} accessibilityElementsHidden importantForAccessibility="no-hide-descendants">‹</Text>
          </TouchableOpacity>
          ...
          {calendarDays.map((day, i) => {
            ...
            return (
              <TouchableOpacity key={i} style={[s.calCell, isSelected && s.calCellSelected]}
                activeOpacity={0.7}
                accessibilityRole="button"
                accessibilityLabel={`${MONTH_NAMES[viewMonth - 1]} ${day}${hasEntry ? ', has entry' : ''}${isToday ? ', today' : ''}`}
            ...
            {entries.slice(-10).reverse().map((entry, i) => {
              ...
              return (
                <TouchableOpacity key={i} style={[s.recentCard, { backgroundColor: colors.card, borderColor: colors.border }]} activeOpacity={0.7}
                  accessibilityRole="button"
                  accessibilityLabel={`Entry from ${new Date(entry.date + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}`}
