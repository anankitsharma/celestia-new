...
// V1 PDF tab structure: Today / Connections / Ask / Profile.
// Chart and Reports both accessed via Profile (Stack.Screen registrations).
// Today is first to establish "this is a daily reflection app" framing.
const TABS = [
  { icon: Star, label: 'Today', name: 'Today' },
  { icon: Users, label: 'Connections', name: 'Connections' },
  { icon: Sparkles, label: 'Ask', name: 'Ask' },
...
export default function TabBar({ state, navigation }) {
  ...
  // Hide tab bar when Ask is active (immersive chat mode)
  const currentRoute = state.routes[state.index]?.name;
  if (keyboardVisible || currentRoute === 'Ask') return null;
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  if (route.name === 'Ask') {
                    ...
                    navigation.navigate('Ask', { previousTab: currentRoute });
                  } else {
              ...
              >
                <View style={styles.iconContainer}>
                  {active && <View style={[styles.pill, { backgroundColor: colors.accentDim }]} />}
                  <Icon
                    size={24}
