...
export default function ChartWheel({ size = 276, planets, aspects, lightMode = false }) {
  ...
  // Rotate chart so ascendant sign starts at left (180°)
  const rotationOffset = ascIdx * 30;

  // Convert absolute degree to chart angle (0° Aries = top, clockwise)
  ...
  return (
    <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {signPositions.map((sp, i) => {
        ...
        return (
          <SvgText key={i} x={c + 119 * Math.cos(a)} y={c + 119 * Math.sin(a)}
            textAnchor="middle" fontSize={9.5} fill={signLabelColor}>
            {sp.sign}
          </SvgText>
        );
      })}
      ...
      {resolvedPlanets.map((p, i) => {
        ...
        return (
          <G key={i}>
            ...
            <SvgText x={x} y={y + 3} textAnchor="middle" fontSize={10} fill={p.color}>{p.glyph}</SvgText>
          </G>
        );
      })}
      ...
      <SvgText x={c} y={c - 2} textAnchor="middle" fontSize={17} fill={ascGlyphColor}>{ascSignSymbol}</SvgText>
      <SvgText x={c} y={c + 12} textAnchor="middle" fontSize={7} fill={ascLabelColor}>ASC</SvgText>
    </Svg>
  );
}
